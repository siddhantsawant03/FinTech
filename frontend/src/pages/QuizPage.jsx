...
export default function QuizPage() {
  const { setQuizAnswers, runAllocation, goToStep, isCalculating } = useStore(
    (s) => ({
  ...
  useEffect(() => {
    ...
    axios
      .get("https://wealthier-ft.onrender.com/api/allocation/questions")
  ...
  const handleNext = async () => {
    ...
    const newAnswers = [
      ...answers,
      { questionId: q.id, answer: selected.label, score: selected.score },
    ];
        ...
        try {
          ...
          toast.error(
            err.response?.data?.error ||
              "Allocation failed. Check backend connection.",
          );
        }
      }
    }, 300);
  ...
  const getRiskHint = (score) => {
    ...
    if (score >= 30)
      return { label: "Moderate Conservative", color: "#3b82f6" };
    ...
    <div className={styles.page}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.topBar}>
          ...
          {hint && (
            <div
              className={styles.liveRisk}
              style={{ borderColor: hint.color + "40", color: hint.color }}
            >
              <div
